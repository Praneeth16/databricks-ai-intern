...
def test_normalize_args_falls_back_on_recursion_error():
    """Codex review caught this gap: ``json.loads`` can raise
    ``RecursionError`` on deeply nested payloads (subclass of
    ``RuntimeError``, not the JSON exception family), which the original
    ``except (JSONDecodeError, TypeError, ValueError)`` clause missed.
    Doom-loop detection would crash mid-turn instead of falling back to
    the raw string hash like the pre-normalisation code did.
    """
    # Build a deeply-nested string that exceeds CPython's default recursion
    # limit when json.loads tries to parse it. 2000 nestings is well past
    # the default sys.getrecursionlimit() of 1000.
    deep = "[" * 2000 + "]" * 2000
    out = _normalize_args(deep)
    # Must not raise. Fallback returns the raw input unchanged.
    assert out == deep


def test_hash_args_resilient_to_pathological_input():
    """End-to-end: pathological input goes through _hash_args without
    raising. We don't assert a specific hash — just that the function
    completes and returns a 12-char hex string.
    """
    deep = "[" * 2000 + "]" * 2000
    h = _hash_args(deep)
