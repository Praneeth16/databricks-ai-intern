...
def _parse_build_result(output: str) -> dict[str, Any]:
    ...
    if not m:
        tail = (output or "")[-800:]
        raise _ToolError(f"no '{_BUILD_SENTINEL}<b64>' sentinel in build output — the build likely "
                         f"failed before emitting a result. Output tail:\n{tail}")
    try:
...
def _validate_build_result(plan: dict[str, Any], result: dict[str, Any]) -> None:
    ...
    if not result.get("smoke_passed"):
        # Surface the structured failure (stage/error/log tail) the build emitted.
        bits = [f"stage={result.get('stage')}", f"error={result.get('error')}"]
        if result.get("vllm_log_tail"):
            bits.append(f"vLLM log tail:\n{result['vllm_log_tail']}")
        raise _ToolError("build failed before a deployable artifact — " + " | ".join(bits))
...
# Runs as a serverless-GPU NOTEBOOK task: results flow back via dbutils.notebook.exit()
# (the notebook_output.result channel), NOT stdout.
...
SENTINEL = {json.dumps(_BUILD_SENTINEL)}


def _emit(result):
    result["plan_hash"] = PLAN_HASH
    dbutils.notebook.exit(SENTINEL + base64.b64encode(json.dumps(result).encode()).decode())  # noqa: F821
...
try:
    ...
    # 1) Materialize weights (HF id -> download; local/Volume path -> reuse).
    ...
    if not os.path.exists(os.path.join(ARTIFACTS_PATH, "config.json")):
        _emit({{"smoke_passed": False, "stage": "validate", "error": "no config.json in artifact"}})
    names = sorted(os.listdir(ARTIFACTS_PATH))
    manifest = hashlib.sha256(
        "".join(f"{{n}}:{{os.path.getsize(os.path.join(ARTIFACTS_PATH, n))}}" for n in names).encode()
    ).hexdigest()

    # 3) Local vLLM smoke (TP=1). Stock vLLM serves the OpenAI route
    #    /v1/chat/completions — /invocations only exists on the deployed SOD.
    import requests
    logf = open("vllm.log", "w")
    proc = subprocess.Popen(["bash", "-lc", SMOKE_ENTRYPOINT], stdout=logf,
                            stderr=subprocess.STDOUT, start_new_session=True)
    smoke_passed, err, ready = False, None, False
    try:
        deadline = time.time() + 1200
        while time.time() < deadline:
            if "Application startup complete" in _log_tail():
                ready = True
                break
            if proc.poll() is not None:
                err = "vLLM process exited during startup"
                break
            time.sleep(5)
        if ready:
            try:
                ...
                smoke_passed = r.status_code == 200 and bool(content)
                if not smoke_passed:
                    err = f"smoke HTTP {{r.status_code}}: {{r.text[:300]}}"
            except Exception as e:
                err = f"smoke request failed: {{e}}"
        elif err is None:
            err = "vLLM did not reach 'Application startup complete' within timeout"
    finally:
        subprocess.run(["pkill", "-f", "vllm.entrypoints.openai.api_server"])

    if not smoke_passed:
        _emit({{"smoke_passed": False, "stage": "smoke", "error": err, "vllm_log_tail": _log_tail()}})
    ...
    _emit({{
        "smoke_passed": True, "registered_model_name": REGISTERED_NAME,
        "model_version": str(mv.version), "mlflow_run_id": info.run_id,
...
def _poll_ready(wc: Any, name: str, timeout_s: int = 1800, interval_s: int = 20) -> str:
    ...
    while time.monotonic() < deadline:
        try:
            ...
            # The enum value is READY or NOT_READY; compare the trailing token
            # exactly — "NOT_READY".endswith("READY") is True, a false positive.
            if ready is not None and str(ready).split(".")[-1].upper() == "READY":
