...
"""
v5.3 — 3-model Optuna-tuned blend + Optuna-tuned ensemble weights.

  - XGB Optuna 20 trials (encoded cats)
  - LGBM Optuna 20 trials (encoded cats)
  - CatBoost Optuna 12 trials (native cats, GPU - slower)
  - Optuna weight search over 2-var simplex (w_xgb, w_lgbm; w_cb = 1 - others)
    100 trials
...
try:
    ...
    print("\n--- Optuna CatBoost 12 trials ---")
    t0 = time.time()
    sc = optuna.create_study(direction="maximize",
                             sampler=optuna.samplers.TPESampler(seed=42))
    sc.optimize(cb_obj, n_trials=12, show_progress_bar=False)
    ...
    SUB_PATH = f"{DATA_DIR}/submission_v5_3_blend3.csv"
    OOF_PATH = f"{DATA_DIR}/oof_v5_3_blend3.csv"
    ...
    with mlflow.start_run(run_name="f1_pitstops_v5_3_blend3") as run:
        mlflow.log_param("version", "v5_3_blend3")
